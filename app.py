...
def process_bom(df_v, df_d, df_p):
    ...
    # 4. Filtrar Directos (Mantenemos tu lógica, pero aseguramos que el AE23 pase completo)
    def validar_opcion(row):
        # Si no tiene opción, pasa.
        if pd.isna(row['EsOpcion']) or str(row['EsOpcion']).strip() == "" or str(row['EsOpcion']).strip() == "0":
            return True
        # Si es una opción (ej: 1, 2, 3), solo pasa si el SKU fue vendido
    ...
    # 7. Nivel 2: Explosión de Procesados (IGUAL QUE LA MECHADA)
    if not df_a_exp.empty:
        ...
        # FACTOR DE GASTO: El mismo que usaste para la mechada
...
# --- INTERFAZ STREAMLIT ---
st.title("📊 MRP - Auditoría Pollo Panko AE23")

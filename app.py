...
def save_recetas_from_excel(df_directos, df_procesados):
    ...
    # 2. Preparar Procesados (Capturando Porción 1/0 y CantReceta para volumen)
    ...
    try:
        ...
        with engine.connect() as conn:
            # REPLICACIÓN DE LÓGICA MRP: Lote (0) vs Porción (1)
            sql_vista = """
            ...
            costo_calculado_procesados AS (
                SELECT 
                    ...
                    SUM(
                        CASE 
                            WHEN r.porcion_marcador = 1 THEN (r.cant_real * COALESCE(pm.costo_unitario_compra, 0))
                            ELSE ( (r.cant_real / NULLIF(rp.volumen_neto, 0)) * COALESCE(pm.costo_unitario_compra, 0) )
                        END
            ...
            conn.commit()
        st.success("✅ Estructura reconstruida con lógica de Lote vs Porción.")
    except Exception as e:
...
if menu_principal == "Gestión BD":
    ...
    if target_sku:
        with st.expander("⚖️ Auditor de MUC (Integrado)", expanded=True):
            query = text("SELECT sku_ingrediente, nombre_ingrediente, precio_unitario_final, costo_parcial_insumo FROM vista_costo_recetas WHERE codigo_venta = :t")
            df = pd.read_sql(query, engine, params={"t": target_sku})
            if not df.empty: st.dataframe(df)
            else: st.warning("SKU no encontrado.")

        with st.expander("🔍 Auditor de Costos (Cascada)", expanded=True):
            df_c = pd.read_sql(text("SELECT * FROM vista_costo_recetas WHERE codigo_venta = :t"), engine, params={"t": target_sku})
            for _, r in df_c.iterrows():
                col1, col2 = st.columns([1, 4])
                col1.metric("Cant.", f"{r['cant_real']} {r['um_salida']}")
    ...
    if st.button("Generar Informe"):
        if sub_inf == "1: Rentabilidad por Producto":
            q = "SELECT categoria_menu, sku_producto, nombre_producto, SUM(monto_venta_real) as venta, SUM(cantidad_vendida) as cant FROM ventas GROUP BY 1, 2, 3"
            df_v = pd.read_sql(text(q), engine)
            df_r = get_recetario_costeado().groupby('codigo_venta')['costo_parcial_insumo'].sum().reset_index()
            res = pd.merge(df_v, df_r, left_on='sku_producto', right_on='codigo_venta').fillna(0)
            res['rentabilidad'] = res['venta'] - (res['cant'] * res['costo_parcial_insumo'])
            st.dataframe(res.format({'venta': '${:,.0f}', 'rentabilidad': '${:,.0f}'}))

...
st.set_page_config(page_title="MRP Pro v4.5 - Fix Mechada", layout="wide")
...
def process_bom(df_v, df_d, df_p):
    ...
    # 2. Lista de SKUs vendidos para filtrar opciones
    skus_vendidos = set(df_v['SKU'].astype(str).str.strip().str.upper())

    # 3. Filtro de Opciones (Tu lógica original)
    ...
    # 5. LÓGICA DE PROCESADOS (PRO-) CON AJUSTE DE FACTOR SOLICITADO
    es_proc = m1['SKU_INSUMO'].str.startswith('PRO-', na=False)
    df_dir_f = m1[~es_proc].copy()
    df_a_exp = m1[es_proc].copy()

    if not df_a_exp.empty:
        # --- EL AJUSTE SOLICITADO ESTÁ AQUÍ ---
        # Calculamos el factor de gasto directo: (CantEfic / CantReceta)
        # Para Mechada será 20.000 / 10.000 = Factor 2.0
        ...
        # El requerimiento final es la multiplicación directa
    ...
    # 6. Consolidación Final y Normalización
...
# --- INTERFAZ STREAMLIT ---
st.title("👨‍🍳 MRP Gastronómico - Factor Directo")
...
if file:
    try:
        ...
        if all(h in xls.sheet_names for h in ['Ventas', 'Directos', 'Procesados']):
            ...
            st.subheader("📋 Requerimiento de Compras")

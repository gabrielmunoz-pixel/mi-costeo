...
st.set_page_config(page_title="BOM Processor v4.5 - Fix Mechada", layout="wide")
...
def process_bom(df_v, df_d, df_p):
    ...
    # 7. Nivel 2: Explosión de Procesados (LÓGICA MECHADA CORREGIDA)
    if not df_a_exp.empty:
        df_p_ready = df_p.rename(columns={
            ...
            'CantEfic': 'VALOR_EFIC',
            'UM': 'UM_PROC',
            'CantReceta': 'CANT_REC_PROC' # Importante para el factor
        })

        # UNIENDO CON LA HOJA DE PROCESADOS
        m2 = pd.merge(df_a_exp, df_p_ready, left_on='SKU_INSUMO', right_on='SKU_PROC_PADRE', how='left')
        
        # --- CAMBIO SOLICITADO: Lógica de Factor Directo ---
        # En lugar de sumar lotes, calculamos el factor: (CantEfic / CantReceta)
        # Para la Plateada de la mechada será: 20000 / 10000 = 2.0
    ...
    # 8. CONSOLIDACIÓN Y NORMALIZACIÓN (Tu versión original)
    ...
    # Normalización de Identidad (SKU es Ley)
    ...
    # Agrupación por SKU y UM
    resumen = consolidado.groupby(['SKU_OUT', 'UM_OUT'], as_index=False).agg({
        'CANT_OUT': 'sum',
        'ING_OUT': 'first'  # Prime el SKU, toma el primer nombre
    })

    # Conversión final a Kilos/Litros
...
st.title("📊 MRP Modular - Verificación Mechada")
...
if file:
    try:
        ...
        if all(h in xls.sheet_names for h in required):
            ...
            st.subheader("📋 Consolidado Normalizado")
            st.dataframe(resultado.style.format({"Total (Kg/L)": "{:,.3f}"}), use_container_width=True)
            
            buf = io.BytesIO()
            with pd.ExcelWriter(buf, engine='openpyxl') as writer:
                resultado.to_excel(writer, index=False)
            st.download_button("📥 Descargar Reporte", buf.getvalue(), "requerimiento_compras.xlsx")
        else:
            st.error(f"Faltan hojas: {required}")

...
st.set_page_config(page_title="MRP Original - Fix Pollo Panko", layout="wide")
...
def process_bom(df_v, df_d, df_p):
    ...
    # 4. Filtrar Directos según EsOpcion (Validación Original)
    def validar_opcion(row):
        if pd.isna(row['EsOpcion']) or str(row['EsOpcion']).strip() == "" or str(row['EsOpcion']).strip() == "0":
            return True
        if str(row['EsOpcion']).strip() == "4": # Caso de recetas citadas
            return True
    ...
    # 7. Nivel 2: Explosión de Procesados (MEJORA PARA POLLO PANKO)
    if not df_a_exp.empty:
        df_p_ready = df_p.rename(columns={
            ...
            'Porcion': 'PORCION_PROCESADO'
        })

        # UNIENDO CON LA HOJA DE PROCESADOS
        m2 = pd.merge(df_a_exp, df_p_ready, left_on='SKU_INSUMO', right_on='SKU_PROC_PADRE', how='left')
        
        # --- LÓGICA DE SUSTITUCIÓN: Si CantReal es 1, usamos la Porción del Procesado ---
        # Esto soluciona que el AE23 dé 0.012kg cuando debería dar >2.5kg
        def calcular_requerimiento(row):
            # Si en Directos dice 1, pero es un PRO-, asumimos que la cantidad real está en la columna Porcion de Procesados
            cantidad_base = row['PORCION_PROCESADO'] if row['CantReal'] == 1 else row['CantReal']
            return row['CANT_VENTA'] * cantidad_base

        m2['REQ_CORREGIDO'] = m2.apply(calcular_requerimiento, axis=1)
        
        # Factor de Gasto (Misma lógica que la Mechada)
        m2['FACTOR_GASTO'] = m2['VALOR_EFIC'] / m2['CANT_REC_PROC']
        m2['TOTAL_FINAL'] = m2['REQ_CORREGIDO'] * m2['FACTOR_GASTO']
    ...
    # Requerimiento para insumos directos
...
st.title("📊 MRP Gastronómico - Auditor de Procesados")
...
if file:
    try:
        ...
        if all(h in xls.sheet_names for h in ['Ventas', 'Directos', 'Procesados']):
            ...
            st.download_button("📥 Descargar Reporte Excel", buf.getvalue(), "mrp_consolidado.xlsx")
            
        else:
            st.error("Faltan hojas críticas: Ventas, Directos o Procesados.")
